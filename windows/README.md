...
dotnet test  windows/ServerMonitor.slnx      # 405 通过，14 条 live 用例跳过
...
| `tests/` | 405 条：解析器、数据库、轮询循环、SFTP 路径、离屏渲染、live |
...
```powershell
dotnet build windows/ServerMonitor.slnx
dotnet test  windows/ServerMonitor.slnx      # 405 pass, 14 live cases skipped
dotnet run --project windows/src/ServerMonitor.App
